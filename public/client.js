...
const renderCats = (data, template, main) => {  
  ...
  data.forEach((cat, index) => {    
    ...
    stars.textContent = `Cuteness rating: ${'😻'.repeat(Math.floor(Math.random() * 5) + 1)}`;    
    const price = catContainer.querySelector('.price');    
    price.textContent = `Price range: ${'💰'.repeat(Math.floor(Math.random() * 3) + 1)}`;            
    // Outlink
    const a = catContainer.querySelector('.outlink');
    a.href = `/forward?url=${encodeURIComponent(cat.url)}`;
    ...
    map.dataset.src = `https://maps.googleapis.com/maps/api/staticmap
        ...
        &format=png
        &markers=size:small%7Ccolor:0xfffb00%7C${lat},${long}`.replace(/\n\s*/g, '');
...
const loadOffers = (offers) => {
  ...
  .then(text => {
    ...
    numbers.splice(-1, 1);  
    numbers.map(text => isNaN(parseInt(text, 10)) ? text : parseInt(text + '9', 10))
    .sort((a, b) => a - b)
    .map((number, index) => {
      const li = document.createElement('li');
      li.textContent = /\d+/.test(number) ? `From $${number} at Affiliate ${index}` : number;
...
const lazyLoadInit = (root = false) => {        
  const things = root ?
        root.querySelectorAll('.lazyload') : document.querySelectorAll('.lazyload');
...
const firstTimeSetup = () => {
  ...
  if ('serviceWorker' in navigator) {
    ...
    .then(registration => console.log(`Service worker registered for scope ${registration.scope}`))
    .catch(e => console.error(e));
  }
};
